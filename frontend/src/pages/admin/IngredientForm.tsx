...
const IngredientForm: React.FC = () => {
  ...
  const [editingIngredient, setEditingIngredient] =
    useState<IngredientView | null>(null)
  const [editName, setEditName] = useState<string>("")
  // Удаление
  const [showConfirmDeleteModal, setShowConfirmDeleteModal] =
    useState<boolean>(false)
  const [ingredientToDeleteId, setIngredientToDeleteId] = useState<
    number | null
  >(null)
  const [ingredientToDeleteName, setIngredientToDeleteName] = useState<
    string | null
  >(null)
  // Состояния формы
    ...
    <div className="p-4 max-w-7xl mx-auto">
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Удалить ингредиент "{ingredientToDeleteName}"?
            </DialogTitle>
          </DialogHeader>
          <div className="py-4">
            <p>
              Вы уверены, что хотите удалить этот ингредиент? Это действие
              нельзя отменить.
            </p>
          </div>
          <div className="flex justify-end gap-2">
