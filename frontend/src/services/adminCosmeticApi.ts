...
export const getCosmeticById = async (
  id: number
): Promise<CosmeticResponse> => {
  const response = await apiClient.get(
    `/api/getCosmeticsById/${id}?isAllData=true`
  )
  return response.data
}
...
export const updateCosmetic = async (id: number, data: { name: string }) => {
  try {
    const response = await apiClient.put(
      `/admin/cosmetic/updateCosmetic/${id}`,
      data
    )
    return response.data
  } catch (error) {
...
export const updateCosmeticCatalog = async (
  cosmeticId: number,
  catalogId: number
): Promise<void> => {
  const response = await apiClient.put(
    `/admin/cosmetic/updateCosmeticCatalog/${cosmeticId}`,
    {
      catalogId,
    }
  )
